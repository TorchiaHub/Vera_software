...
import { LocalPCMonitor } from './components/local-pc-monitor-v2';
...
import { Leaf, Monitor, Smartphone, Tablet, User, BarChart3, Settings, Activity, ChevronLeft, ChevronRight, MessageSquare, Trophy, Laptop, Wifi } from 'lucide-react';
...
function App() {
  ...
  const { currentDevice, devices } = useDeviceManager();
  ...
  const [userCO2Saved, setUserCO2Saved] = useState(45.8);
  const [userBottlesSaved, setUserBottlesSaved] = useState(91.6);
    ...
    invoke('get_settings')
      .then(settings => {
        ...
        if (settings && typeof settings === 'object' && 'theme' in settings) {
          setTheme((settings as any).theme);
        }
      })
      .catch(err => {
        clearTimeout(timeoutId);
